...
// Copyright (c) 2016-2018 The DSC developers
...
public:
    ...
    bool WriteZDSCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDSCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDSCSeed_deprecated(uint256& seed);
    bool EraseZDSCSeed();
    bool EraseZDSCSeed_deprecated();

    bool WriteZDSCCount(const uint32_t& nCount);
    bool ReadZDSCCount(uint32_t& nCount);
