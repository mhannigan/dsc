...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::DSC);
    ...
    if (!settings.contains("nAnonymizeDscAmount"))
        settings.setValue("nAnonymizeDscAmount", 1000);

    nAnonymizeDscAmount = settings.value("nAnonymizeDscAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDscAmount"))
        SoftSetArg("-anonymizedscamount", settings.value("nAnonymizeDscAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in dsc.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDscAmount:
            return QVariant(nAnonymizeDscAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDscAmount:
            nAnonymizeDscAmount = value.toInt();
            settings.setValue("nAnonymizeDscAmount", nAnonymizeDscAmount);
            emit anonymizeDscAmountChanged(nAnonymizeDscAmount);
            break;
        case CoinControlFeatures:
