...
#ifndef DSC_ZDSCWALLET_H
#define DSC_ZDSCWALLET_H
...
public:
    CzDSCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZDSC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZDSC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //DSC_ZDSCWALLET_H
