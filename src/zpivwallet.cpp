...
CzDSCWallet::CzDSCWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDSCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDSCSeed_deprecated()) {
                    LogPrintf("%s: Updated zDSC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzDSCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDSCCount(nCountLastUsed);
    else if (!walletdb.ReadZDSCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDSCWallet::Lock()
{
    seedMaster = 0;
}

void CzDSCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDSCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDSC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDSCWallet::LoadMintPoolFromDB()
{
...
void CzDSCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDSCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDSCWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzDSCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDSC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzDSCWallet::SeedToZDSC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDSCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDSCWallet::GenerateDeterministicZDSC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDSCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDSCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
