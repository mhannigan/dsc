...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 DSC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zDSC send
enum ZerocoinSpendStatus {
    ZDSC_SPEND_OKAY = 0,                            // No error
    ZDSC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZDSC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZDSC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZDSC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZDSC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZDSC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZDSC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZDSC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZDSC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZDSC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZDSC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZDSC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZDSC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZDSC_SPENT_USED_ZDSC = 14,                      // Coin has already been spend
    ZDSC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZDSC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZDSCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzDSCWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzDSCTracker> zpivTracker;
    ...
    void setZWallet(CzDSCWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzDSCTracker>(new CzDSCTracker(strWalletFile));
    }

    CzDSCWallet* getZWallet() { return zwalletMain; }
    ...
    /** zDSC reset */
    boost::signals2::signal<void()> NotifyzDSCReset;
